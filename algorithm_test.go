...
func TestAlgorithmKeyPredicatesRejectEveryIndependentBoundary(t *testing.T) {
	...
	for _, key := range []*ecdsa.PrivateKey{
		...
		{PublicKey: ecdsa.PublicKey{Curve: elliptic.P256(), X: big.NewInt(1), Y: big.NewInt(1)}, D: big.NewInt(1)}, //nolint:staticcheck // Deliberately constructs invalid key material.
...
func TestAlgorithmsRejectCorrectTypesWithInvalidKeyMaterial(t *testing.T) {
	...
	base := []byte("base")
	p256 := &ecdsa.PrivateKey{PublicKey: ecdsa.PublicKey{Curve: elliptic.P256(), X: big.NewInt(1), Y: big.NewInt(1)}, D: big.NewInt(0)} //nolint:staticcheck // Deliberately constructs invalid key material.
	p384 := &ecdsa.PrivateKey{PublicKey: ecdsa.PublicKey{Curve: elliptic.P384(), X: big.NewInt(1), Y: big.NewInt(1)}, D: big.NewInt(0)} //nolint:staticcheck // Deliberately constructs invalid key material.
